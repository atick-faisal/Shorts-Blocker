...
- **Open Source Licenses Crash**: Fixed a fatal `NullPointerException` in `OssLicensesActivity.onCreate` when opening the "Licenses" screen ([#79](https://github.com/atick-faisal/Shorts-Blocker/issues/79))
  - The `oss-licenses-plugin` was declared but never applied to the app module, so the license resources it reads at runtime were never generated
  - Applied the plugin in `app/build.gradle.kts` so license data is generated correctly
  - Wrapped the licenses screen launch in a try/catch as defense in depth against future regressions
